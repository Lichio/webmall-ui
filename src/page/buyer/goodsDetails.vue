    ...
    <div class="gray-box">
      ...
      <div class="banner">
        ...
        <div class="num">
          ...
          <buy-num @edit-num="editNum" :limit="Number(product.quantity)"></buy-num>
        </div>
        <div class="buy">
          <y-button text="加入购物车"
                    @btnClick="addCart(product.commodityId,product.price,product.name,product.productImageBig)"
                    classStyle="main-btn"
                    style="width: 145px;height: 50px;line-height: 48px"></y-button>
          <y-button text="现在购买"
                    @btnClick="checkout(product.commodityId)"
...
<script>
import { commodityDetails, addToCart } from '/api/index.js'
  ...
  methods: {
    ...
    _productDet (commodityId) {
      commodityDetails(
        ...
        this.big = result.picture
      })
    },
    addCart (id, price, name, img) {
      addToCart({buyerId: getStore('id'), commodityId: id, quantity: this.productNum}).then(res => {
        console.log(res)
        if (res.code === 200) {
          this.open('通知', '加入购物成功，请继续购物！')
        }
      // 并不重新请求数据
