          ...
          <div class="ui-cart">
            <div>
              ...
              <div class="cart-table" v-for="(item,i) in cartList" :key="i">
                <div class="cart-group divide pr" :data-productid="item.commodityId">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      <!--勾选-->
                      <div class="items-choose">
                      <span class="blue-checkbox-new " :class="{'checkbox-on':item.checked } "
                      ...
                      <div class="items-thumb fl">
                        <img :alt="item.name"
                             :src="item.picture">
                        <a @click="goodsDetails(item.commodityId)" :title="item.name" target="_blank"></a>
                      </div>
                      <!--信息-->
                      <div class="name hide-row fl">
                        <div class="name-table">
                          <a @click="goodsDetails(item.commodityId)" :title="item.name" target="_blank"
                             v-text="item.name"></a>
                          <!-- <ul class="attribute">
                            <li>白色</li>
                          </ul> -->
                      ...
                      <!--删除按钮-->
                      <div class="operation">
                        <a class="items-delete-btn" @click="cartdel(item.commodityId)"></a>
                      </div>
                      <!--商品数量-->
                      <div>
                        <!--总价格-->
...
<script>
import { shopcartList, deleteCart } from '/api/index.js'
...
export default {
  ...
  computed: {
    ...
    checkAllFlag () {
      ...
      return checkedcount === this.cartList.length
    },
    // 勾选的数量
    // checkedCount () {
    //   var i = 0
    ...
    checkchecked1 () {
      return function (id) {
        return getStore('checked-status-' + id) === '1'
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    editCheckAll () {
      ...
      if (this.checkAllFlag) {
        this.cartList && this.cartList.forEach(item => {
          // setStore('checked-status-' + item.commodityId, '1')
          item.checked = false
        })
      } else {
        this.cartList && this.cartList.forEach(item => {
    ...
    editCart (type, item) {
      if (type && item) {
        // let checked = getStore('checked-status-' + item.commodityId)
        // let productId = item.commodityId
        ...
        if (type === 'check') {
          ...
          // setStore('checked-status-' + item.commodityId, newChecked)
    ...
    // 删除整条购物车
    cartdel (productId) {
      deleteCart({params: {buyerId: this.userId, commodityId: productId}}).then(res => {
        console.log(res)
        // removeStore('checked-status-' + productId)
        let cartt = this.cartList
        cartt.forEach((item, i) => {
          if (item.commodityId === productId) {
            cartt.splice(i, 1)
          }
        })
        this.cartList = cartt
    ...
    checkout () {
      ...
      this.$router.push({path: '/buyer/checkout'})
    },
    // delChecked () {
    //   getCartList({userId: getStore('buyerId')}).then(res => {
    //     if (res.success === true) {
    ...
    // },
    _getCartList () {
      shopcartList({buyerId: this.userId}).then(res => {
        console.log(res)
        if (res.code === 200) {
