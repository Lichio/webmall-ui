        ...
        <div slot="content">
          <ul class="address-item-list clearfix">
            <li v-for="(item,i) in addList"
                ...
                class="address pr"
                :class="{checked:true}"
                @click="chooseAddress(item.id, item.username, item.username, item.address)"
                >
           <span v-if="addressId === item.id" class="pa">
             ...
             <path
              ...
              <div class="operation-section">
                ...
                <span class="delete-btn" style="font-size:12px" :data-id="item.id" @click="del(item.addressId)">删除</span>
              </div>
            </li>
          ...
          <div class="box-inner ui-cart">
            <div>
              ...
              <div class="cart-table" v-for="(item,i) in cartList" :key="i" >
                <div class="cart-group divide pr" :data-productid="item.commodityId" v-if="item.checked">
...
<script>
import { commodityDetails, submitOrder } from '/api/index.js'
...
export default {
  ...
  methods: {
    ...
    _addressList () {
      ...
      // this.addList.push({id: this.userId})
      // this.addList.push({address: username + "'s address"})
      this.addressId = this.userId
      this.userName = username
      this.tel = username
      this.streetName = username + "'s address"
      ...
      this.cartList.forEach((item) => {
        if (item.checked) {
          submitOrder({
            ...
            buyerId: this.userId,
            address: this.addList[0].address
          }).then(res => {
            console.log(res)
          })
        }
    ...
    _productDet (productId) {
      commodityDetails({commodityId: productId}).then(res => {
        console.log(res)
        let item = res.data
        item.checked = true
        //   item.picture = item.productImageBig
  ...
  created () {
    ...
    if (query.commodityId && query.num) {
      this.productId = query.commodityId
      this.num = query.num
